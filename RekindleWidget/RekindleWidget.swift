...
struct RekindleTimelineProvider: TimelineProvider {
    ...
    private func fetchEntry() -> RekindleEntry {
        do {
            ...
            // Includes both standard picks and favorites (favorites count toward the total,
            // same as the app). Favorites are distinguished by a gold avatar in the views.
            var recommendations = try context.fetch(descriptor)

            // If there are no STANDARD picks yet today, seed them so the widget isn't blank
            // before the app has run. (Favorites are only ever generated by the app/background
            // task — never seeded here.) Uses the same weighted selection as the app.
            let hasStandardPick = recommendations.contains { !$0.isFavorite }
            if !hasStandardPick {
                ...
                if let settings = try? context.fetch(settingsDescriptor).first,
                   !settings.isCurrentlyPaused, settings.isTodayScheduled {
                    ...
                    let eligible = allContacts.filter { contact in
                        guard !contact.isBlocked, !contact.isFavorite else { return false }
